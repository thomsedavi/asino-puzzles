...
export const getRectangleReferenceRow = (puzzle: AsinoPuzzle, rectangleReference: AsinoRectangleReference, key: string, depth: number, update: (value: AsinoRectangleReference) => void): JSX.Element => {
  ...
  return <div key={rowKey} style={{ marginBottom: '1em' }}>
    ...
    {getNumberRow(puzzle, rectangleReference.value?.rectangle?.[X], `${rowKey}x`, depth + 1, (value: AsinoNumber | undefined) => update({ ...rectangleReference, value: { rectangle: { ...rectangleReference.value?.rectangle, [X]: value ?? { integer: { value: 1 } } } } }))}
    {getNumberRow(puzzle, rectangleReference.value?.rectangle?.[Y], `${rowKey}y`, depth + 1, (value: AsinoNumber | undefined) => update({ ...rectangleReference, value: { rectangle: { ...rectangleReference.value?.rectangle, [Y]: value ?? { integer: { value: 1 } } } } }))}
    {getNumberRow(puzzle, rectangleReference.value?.rectangle?.[Width], `${rowKey}width`, depth + 1, (value: AsinoNumber | undefined) => update({ ...rectangleReference, value: { rectangle: { ...rectangleReference.value?.rectangle, [Width]: value ?? { integer: { value: 1 } } } } }))}
    {getNumberRow(puzzle, rectangleReference.value?.rectangle?.[Height], `${rowKey}height`, depth + 1, (value: AsinoNumber | undefined) => update({ ...rectangleReference, value: { rectangle: { ...rectangleReference.value?.rectangle, [Height]: value ?? { integer: { value: 1 } } } } }))}
  </div>;
}
