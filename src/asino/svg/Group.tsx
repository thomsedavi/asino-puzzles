...
export const drawGroup = (groups: (AsinoGroupReference | undefined)[], references: References, solution: Solution, key: string): JSX.Element => {
  ...
  groups.forEach((group: AsinoGroupReference | undefined) => {
    ...
    if (matrix !== undefined) {
      ...
      matrix[A] !== undefined && (bMatrix = getValueFromNumber(getNumberFromAsinoNumber(matrix[B], references.clone()), references.clone(), true) ?? 0);
      matrix[A] !== undefined && (cMatrix = getValueFromNumber(getNumberFromAsinoNumber(matrix[C], references.clone()), references.clone(), true) ?? 0);
      matrix[A] !== undefined && (dMatrix = getValueFromNumber(getNumberFromAsinoNumber(matrix[D], references.clone()), references.clone(), true) ?? 1);
      matrix[A] !== undefined && (eMatrix = getValueFromNumber(getNumberFromAsinoNumber(matrix[E], references.clone()), references.clone(), true) ?? 0);
      matrix[A] !== undefined && (fMatrix = getValueFromNumber(getNumberFromAsinoNumber(matrix[F], references.clone()), references.clone(), true) ?? 0);
    }

    if (translate !== undefined) {
      translate[X] !== undefined && (xTranslate = getValueFromNumber(getNumberFromAsinoNumber(translate[X], references.clone()), references.clone(), true) ?? 1);
      translate[Y] !== undefined && (yTranslate = getValueFromNumber(getNumberFromAsinoNumber(translate[Y], references.clone()), references.clone(), true) ?? 1);
    }

    if (scale !== undefined) {
      scale[X] !== undefined && (xScale = getValueFromNumber(getNumberFromAsinoNumber(scale[X], references.clone()), references.clone(), true) ?? 1);
      scale[Y] !== undefined && (yScale = getValueFromNumber(getNumberFromAsinoNumber(scale[Y], references.clone()), references.clone(), true) ?? 1);
    }

    if (rotate !== undefined) {
      rotate[A] !== undefined && (aRotate = getValueFromNumber(getNumberFromAsinoNumber(rotate[A], references.clone()), references.clone(), true) ?? 0);
  ...
  if (xTranslate !== undefined || yTranslate !== undefined) {
    transform = (transform === undefined ? '' : `${transform} `) + `translate(${(xTranslate ?? 1) * 5040} ${(yTranslate ?? xTranslate ?? 1) * 5040})`;
  }
  ...
  if (aRotate !== undefined || xRotate !== undefined || yRotate !== undefined) {
    transform = (transform === undefined ? '' : `${transform} `) + `rotate(${aRotate ?? 0} ${(xRotate ?? 0) * 5040} ${(yRotate ?? xRotate ?? 0) * 5040})`;
  }
