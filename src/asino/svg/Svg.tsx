...
export const drawLayer = (puzzle: AsinoPuzzle, solution: Solution, layer: AsinoLayer, scale: Number, key: string, selectedObjectId?: string): JSX.Element => {
  if (layer.lineId !== undefined || layer.line !== undefined) {
    ...
    return drawPath([layerPath, layer.path], [...(puzzle.numbers ?? [])], puzzle.defaults?.[StrokeWidth] ?? { number: { numerator: 1, denominator: 200 } }, scale, key);
  } else {
...
export const drawSvg = (puzzle: AsinoPuzzle, solution: Solution, setSelectedCollectionId: (objectId: string) => void, setSelectedObjectId: (objectId: string) => void, selectedObjectId?: string): JSX.Element => {
  ...
  puzzle.layers?.forEach((layer: AsinoLayer, layerIndex: number) => {
    layers.push(drawLayer(puzzle, solution, layer, 1, `layer${layerIndex}`, selectedObjectId));
  });
