...
export const drawInterface = (interfaces: (AsinoInterface | undefined)[], objectIds: (string | undefined)[], numbers: AsinoNumber[], defaultInterfaceWidthValue: AsinoNumber, defaultInterfaceHeightValue: AsinoNumber, index: number, selectedObjectId?: string): JSX.Element => {
...
export const drawInterfaceInteractive = (interfaces: (AsinoInterface | undefined)[], objectIds: (string | undefined)[], numbers: AsinoNumber[], defaultInterfaceWidthValue: AsinoNumber, defaultInterfaceHeightValue: AsinoNumber, index: number, setSelectedObjectId: (objectId: string) => void): JSX.Element => {
  const x = getNumberFromLayer(interfaces, numbers, X, XId, { number: 0 });
  const y = getNumberFromLayer(interfaces, numbers, Y, YId, { number: 0 });
  const width = getNumberFromLayer(interfaces, numbers, Width, WidthId, defaultInterfaceWidthValue);
  const height = getNumberFromLayer(interfaces, numbers, Height, HeightId, defaultInterfaceHeightValue);
  ...
  return <rect
    ...
    strokeWidth={getGridValue({numerator: 1, denominator: 200})}
    onClick={() => { interfaceObjectId !== undefined && setSelectedObjectId(interfaceObjectId) }}
  />;
}